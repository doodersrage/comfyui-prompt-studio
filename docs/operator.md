...
- **Phone companion** — `/m` is Capture → Queue → Gallery → Play stills. Use **Desk** for Fitting, Day, Moodboard, and the full Cut film loop. Header desk bridge deep-links **Campaign** (`/play`), **Day**, and **Fitting**; `/m/play` also offers Continue Day / Play campaign on desk. Add to Home Screen via the mobile manifest when you want a phone-sized dock.
