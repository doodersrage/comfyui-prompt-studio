...
| `NEXT_PUBLIC_PROMPT_NSFW_GENERATOR_ENABLED` | `false`                     | Same lockout for nav/UI (rebuild required). `next.config` copies the server flag when this is unset                                                                                    |
...
| `PROMPT_ENGINE`                        | `comfyui`                        | `comfyui` (default), `diffusers`, `fal`, `replicate`, `openai`, `gemini`, or `grok`                                                                                                    |
