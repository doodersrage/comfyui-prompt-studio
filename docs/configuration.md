...
| `PROMPT_ENGINE`                        | `comfyui`                        | `comfyui` (default), `diffusers` (stills-only experimental), `fal`, `replicate`, `openai`, `gemini`, `grok`, or `runway`. Fal/Replicate/Grok/Gemini/Runway queue clips; ChatGPT and Diffusers stay stills. |
