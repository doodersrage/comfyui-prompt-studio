...
- **Roleplay** — `/roleplay` (More tools in Simple mode): pick an archetype or custom persona, write a bio, roll four story beats, tap a beat to queue a still in the story reel. **Play as** From bio (default, text-to-image models) or From photo (upload a selfie, pick a gallery still, or reuse the last story still — the model picker switches to edit / img2img checkpoints and each beat queues img2img + identity lock from that reference). **Isolate on white** (default) cuts the subject onto a blank plate so the photo’s scene does not leak through; optional **Setting** seeds a location into bios, beats, and stills. Exclusive **Tone** (Silly / Cinematic / Cozy / Chaotic) and **Content** rating (SFW: Clean, PG-13, Suggestive; Adult: Sultry, Explicit, Raunchy — uncensored; Explicit names sex and anatomy) plus an additive **Gore** toggle. Stills render inline with live Comfy preview; download a zip of `story.md` + stills. Consenting adults only; no minors. Default PG-13.
