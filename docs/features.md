...
- **Moodboard → Scene** — `/moodboard` (Scene nav / ⌘K): stack up to four reference tiles (mood, lighting, location, style, palette) with optional gallery stills; choose a merge template and queue one scene still. **Extract look** builds a session look pack (vision merge when tiles have images, otherwise text cues) and **Use in Fitting / Day / Roleplay** hand off vibe notes + optional wardrobe lock. **Save on Cast** persists named look packs on the character record. Deep link `?character=`; cross-links to Day Planner, Fitting Room, Roleplay, and **Play campaign** (`/play`).
- **Play campaign** — `/play` (Play workspace): guided stepper — Cast → Moodboard → Fitting → Day → Roleplay — with look pack + character state carried through session handoff and saved look packs on Cast.
