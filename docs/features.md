...
- **Play workspace** — sidebar footer / Profile → Appearance: Cast, Roleplay, Gallery, and Queue only (Roleplay mode label in workspace picker). Roleplay routes use a lean rail. Default for a film-first loop; Simple remains the first-run default.
...
- **Cast** — `/characters`: character homes for looks, stills, clips, and the Roleplay film cut. Continue reel uses Video I2V; **Continue in Roleplay** restores the library session when it is still present (otherwise the UI says the session is missing). Roster empty CTA points at Save to Cast.
