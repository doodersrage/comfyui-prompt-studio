...
- **Day Planner** — `/day` (Play bottom bar / Scene nav): pick a Cast character and plan four time slots (Morning → Night) with wardrobe kit, setting, and beat per slot; queue one still or all slots. Deep link `?character=`; cross-links to Fitting Room, Moodboard, and Roleplay.
- **Moodboard → Scene** — `/moodboard` (Scene nav / ⌘K): stack up to four reference tiles (mood, lighting, location, style, palette) with optional gallery stills; choose a merge template and queue one scene still. Deep link `?character=`; cross-links to Day Planner, Fitting Room, and Roleplay.
