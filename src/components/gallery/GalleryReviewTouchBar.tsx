    ...
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-zinc-800/80 bg-zinc-950/95 px-3 py-3 backdrop-blur-md lg:hidden">
      <div className="mx-auto flex max-w-lg items-center justify-between gap-2">
        <button
          type="button"
          onClick={onPrev}
          className={`ui-btn-secondary min-h-11 rounded-xl border border-zinc-900/80 bg-zinc-950/60 backdrop-blur-xs transition hover:bg-violet-500/25 hover:border-violet-500/65 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400/30 text-violet-400`}
        >
          Prev
        </button>
        <div className="flex gap-1">
          {([1, 2, 3, 4, 5] as const).map((rating, idx) => (
            <button
              ...
              className={`min-h-11 min-w-11 rounded-xl border-zinc-800/60 bg-zinc-950/70 backdrop-blur-xs transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400/30 ${ratingColors[idx]}`}
            >
              {rating}
            </button>
          ))}
