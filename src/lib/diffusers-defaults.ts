 ...
 */
export function resolveStudioModelForDiffusersAsset(assetId: string, family?: string): string {
...
export function workshopCropToApi(mode: DiffusersWorkshopCropMode | undefined): boolean | null {
  if (mode === 'always') {
    return true;
  }
  if (mode === 'never') {
