...
export function loraFilenameLooksLikeKleinUltraReal(
  ...
  if (
    /amplifier|canopus|ultrareal\s*photo|ultrarealism|ultrareal.?fine.?tune/i.test(
      name,
    )
  ) {
    return false;
...
function withUltraRealDefaults(
...
export function ensureKleinUltraRealInLibrary(
  ...
  const byId = normalized.find(
    (entry) => entry.id.trim() === KLEIN_ULTRA_REAL_LORA_ID,
  );
  if (byId) {
