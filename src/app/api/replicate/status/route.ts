...
import { fetchReplicateJobStatus } from '@/lib/replicate-client';
...
export async function GET(request: Request) {
  ...
  const status = await fetchReplicateJobStatus(promptId);
  if (!status) {
    return apiError('Replicate status check failed.', 502);
  }
...
export async function POST() {
  return apiMethodNotAllowed(['GET'], '/api/replicate/status');
}
