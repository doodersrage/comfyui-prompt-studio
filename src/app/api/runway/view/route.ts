...
import { ensureRunwayOutput } from '@/lib/runway-client';
...
export async function GET(request: Request) {
  ...
  try {
    const file = await ensureRunwayOutput({
      promptId: searchParams.get('promptId')?.trim() || undefined,
      filename,
      subfolder,
    });
    if (!file) {
      return apiError('Runway image is not available yet.', 404);
    }
    ...
    return apiError(error instanceof Error ? error.message : 'Runway view failed.', 502);
  }
}

export async function POST() {
  return apiMethodNotAllowed(['GET'], '/api/runway/view');
}
