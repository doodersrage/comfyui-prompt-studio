...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Image refine failed.';
    const status = /required|must be|could not read|upload must|too large|8mb/i.test(message)
      ? 400
      : 500;
    const stack =
